...
std::string CxImageFuns::FindExtension(const std::string& name)
{
	int len = name.length();
	int i;
	for (i = len-1; i >= 0; i--){
...
int  CxImageFuns::FindFormat(const std::string& ext)
{
	int type = 0;
...
int  CxImageFuns::GetImageFileType(const std::string& name)
{
