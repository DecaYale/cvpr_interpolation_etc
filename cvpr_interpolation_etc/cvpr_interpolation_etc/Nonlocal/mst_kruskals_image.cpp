...
inline void mst_compute_edges_per_pixel(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int &nr_edge,int y0,int x0,int yt,int xt,int h,int w)
{
	int id0=edges[nr_edge][0]=mst_yx_2_image_index(y0,x0,h,w);
	int idt=edges[nr_edge][1]=mst_yx_2_image_index(yt,xt,h,w);
	if(nr_channel==1)
	...
	else if(nr_channel==3)
	{
		...
		//if(e<DEF_MST_KI_MIN_DISTANT) distance[nr_edge]=0;
...
inline int mst_compute_edges_4neighbor(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int h,int w)
{
	int y0,yt,x0,xt;
	int nr_edge=0;
	for(y0=0;y0<h;y0++)
	{
		...
		{
			xt=x0+1;
			mst_compute_edges_per_pixel(edges,distance,image,nr_channel,nr_edge,y0,x0,yt,xt,h,w);
		}
	}
...
inline int mst_compute_edges_8neighbor(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int h,int w)
{
	int y0,yt,x0,xt;
	int nr_edge=mst_compute_edges_4neighbor(edges,distance,image,nr_channel,h,w);
	for(y0=0;y0<h-1;y0++)
	{
		yt=y0+1;
		for(int x0=0;x0<w-1;x0++)
		{
			xt=x0+1;
			mst_compute_edges_per_pixel(edges,distance,image,nr_channel,nr_edge,y0,x0,yt,xt,h,w);
			mst_compute_edges_per_pixel(edges,distance,image,nr_channel,nr_edge,y0,xt,yt,x0,h,w);
		}
	}
	return(nr_edge);
}

mst_kruskals_image::mst_kruskals_image()
{
	m_image=NULL;
	m_edge=NULL;
	m_distance=NULL;
	...
	m_parent_default=DEF_MST_KI_PARENT_DEFAULT;
}
mst_kruskals_image::~mst_kruskals_image()
{
	clean();
}
void mst_kruskals_image::clean()
{
	freeu_1(m_image); m_image=NULL;
	freei(m_edge); m_edge=NULL;
	freeu_1(m_distance); m_distance=NULL;
	freei_1(m_id_edge); m_id_edge=NULL;

	freei_1(m_parent); m_parent=NULL;
	freei_1(m_nr_child); m_nr_child=NULL;
	freeu_1(m_weight); m_weight=NULL;
	freei(m_children); m_children=NULL;

	freei(m_connected); m_connected=NULL;
	freeu(m_connected_distance); m_connected_distance=NULL;
	freei_1(m_nr_connected); m_nr_connected=NULL;

	freei_1(m_node_id_from_parent_to_child); m_node_id_from_parent_to_child=NULL;
	freei_1(m_rank); m_rank=NULL;
}
int mst_kruskals_image::init(int h,int w,int nr_channel,int nr_neighbor)
{
	clean();
	m_h=h; m_w=w; m_nr_channel=nr_channel; m_nr_neighbor=nr_neighbor;
	m_max_nr_child=m_nr_neighbor-1;
	m_nr_vertices=m_h*m_w;
	if(m_nr_neighbor==DEF_MST_KI_4NR_NEIGHBOR) m_nr_edge=mst_compute_nr_edge_4neighbor(m_h,m_w);
	else m_nr_edge=mst_compute_nr_edge_8neighbor(m_h,m_w);
	m_image=new unsigned char [m_h*m_w*m_nr_channel];
	m_edge=alloci(m_nr_edge,2);
	...
	m_children=alloci(m_nr_vertices,m_max_nr_child);

	m_connected=alloci(m_nr_vertices,m_nr_neighbor);
	m_connected_distance=allocu(m_nr_vertices,m_nr_neighbor);
...
void mst_kruskals_image::init_mst()
{
	int*parent=m_parent;
	for(int i=0;i<m_nr_vertices;i++) *parent++=i;
	memset(m_nr_connected,0,sizeof(int)*m_nr_vertices);
}
int mst_kruskals_image::mst(unsigned char*image,bool print_edges)
{
	timer timer;
	timer.start();
	init_mst();
	...
	if(m_nr_neighbor==DEF_MST_KI_4NR_NEIGHBOR) mst_compute_edges_4neighbor(m_edge,m_distance,m_image,m_nr_channel,m_h,m_w);//find edges
	else mst_compute_edges_8neighbor(m_edge,m_distance,m_image,m_nr_channel,m_h,m_w);
	//timer.fps_display();
	timer.start();
	sort_increase_using_histogram(m_id_edge,m_distance,m_nr_edge);//sorting edges in a nondesc
	//timer.fps_display("sort_increase_using_histogram");
	timer.start();
	kruskal();
...
int mst_kruskals_image::findset(int x)
{
...
void mst_kruskals_image::kruskal()
{
	m_tree_size=0;
...
int mst_kruskals_image::build_tree()
{
	timer timer;
	timer.start();
	int tree_parent=0;
...
void queue_mst::clean()
{
	if(queue!=NULL) delete [] queue; queue=NULL;
}
int queue_mst::init(int len)
{
	if (len<=0)
	{
...
void queue_mst::reinit()
{
	first=0;
	last=-1;
	length=0;
}
void queue_mst::push(int x)
{
	length++;
	queue[++last]=x;
}
