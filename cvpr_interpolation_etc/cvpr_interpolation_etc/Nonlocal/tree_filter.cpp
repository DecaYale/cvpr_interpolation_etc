...
void tree_filter::clean()
{
}
int tree_filter::init(int h,int w,int nr_channel,double sigma_range,int nr_neighbor)
{
...
void tree_filter::update_table(double sigma_range)
{
	sigma_range=max(0.01,sigma_range);
	for(int i=0;i<=DEF_CHAR_MAX;i++) m_table[i]=exp(-double(i)/(DEF_CHAR_MAX*sigma_range));//weight table
}
int tree_filter::build_tree(unsigned char*texture)
{
	m_mst.mst(texture);
...
template<typename T>void tree_filter::combine_tree(T*image_filtered)
{
...
template<typename T>void tree_filter::init_tree_value(T*image,bool compute_weight)
{
...
int tree_filter::filter(float*cost,float*cost_backup,int nr_plane)
{
	memcpy(cost_backup,cost,sizeof(float)*m_h*m_w*nr_plane);
	...
	for(int i=0;i<m_nr_pixel;i++)
	{
		...
		if(nr_child>0)
		{
			float*value_sum=&(cost_backup[id_]);
			for(int j=0;j<nr_child;j++)
			{
				...
				float*value_child=&(cost_backup[id_child_]);
	...
	memcpy(&(cost[tree_parent_]),&(cost_backup[tree_parent_]),sizeof(float)*nr_plane);
	for(int i=1;i<m_nr_pixel;i++)//K_00=f(0,00)[K_0-f(0,00)J_00]+J_00, K_00: new value, J_00: old value, K_0: new value of K_00's parent
	{
		...
		float*value_parent=&(cost[parent_]);//K_0
		float*value_current=&(cost_backup[id_]);//J_00
		float*value_final=&(cost[id_]);
