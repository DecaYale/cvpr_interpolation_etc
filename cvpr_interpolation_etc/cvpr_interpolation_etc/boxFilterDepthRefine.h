...
extern void boxFilterDepthRefine(const cv::Mat & img_L,const cv::Mat & img_R,cv::Mat &coarseDepthMap, cv::Mat &fineDepthMap,cv::Mat & isValidate, int dLevels ,int winWidth,int deviation,double validThreshold = 5,double curvThreshold = -2,double peakRatio = 1.1);
