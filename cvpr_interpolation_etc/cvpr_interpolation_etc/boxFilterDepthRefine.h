...
extern void  boxFilterDepthRefine_test(const cv::Mat & img_L,const cv::Mat & img_R,cv::Mat &coarseDepthMap, const std::vector<bool> & dispSample, cv::Mat &fineDepthMap,cv::Mat & winCostCube, cv:: Mat & isValid, int dLevels ,int winWidth,int deviation,double validThreshold = 5,double curvThreshold = -2,double peakRatio = 1.1);