...
inline void image_zero(unsigned char***out,int h,int w,int d,unsigned char zero=0){memset(out[0][0],zero,sizeof(unsigned char)*h*w*d);}
inline void image_copy(unsigned char***out,unsigned char***in,int h,int w,int d=3){memcpy(out[0][0],in[0][0],sizeof(unsigned char)*h*w*d);}
