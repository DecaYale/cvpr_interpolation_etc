...
void Nonlocal::stereo(const Mat & imgL, Mat & datacost,Mat & labelimg,double sigma,bool rfm)
{
	//	if(!rfm)
	//	{
	//	ZIntImage zimg;
	...
	for(int idx=0;idx<W;idx++)//for(int idx=0;idx<block.m_iWidth;idx++)
		for(int idy=0;idy<H;idy++)//for(int idy=0;idy<block.m_iHeight;idy++)
		{
			...
			m_left[idy][idx][0]= (unsigned char)imgL.at<uchar>(idy,idx);//u1.buffer[0];
