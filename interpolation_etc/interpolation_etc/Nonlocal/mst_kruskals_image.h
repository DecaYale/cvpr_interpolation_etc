...
************************************************************************************/
#ifndef MST_KRUSKALS_IMAGE_H
#define MST_KRUSKALS_IMAGE_H
#define DEF_MST_KI_MIN_DISTANT			1
#define DEF_MST_KI_4NR_NEIGHBOR			4
#define DEF_MST_KI_8NR_NEIGHBOR			8
#define DEF_MST_KI_PARENT_DEFAULT		-1
#define DEF_MST_KI_SIGMA_RANGE			0.1
#include "ctmf.h"
class queue_mst
{
public:
...
class mst_kruskals_image
{
public:
	mst_kruskals_image();
	~mst_kruskals_image();
	void clean();
	int init(int h,int w,int nr_channel,int nr_neighbor=DEF_MST_KI_4NR_NEIGHBOR);
...
private:
	...
	void kruskal();
	int build_tree();
	//void reorder_edges(vector< pair< int, PAIR > >&min_spanning_tree);
	//void print(vector< pair< int, PAIR > >&min_spanning_tree);
};
