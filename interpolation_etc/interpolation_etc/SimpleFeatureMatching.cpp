...
//img: uchar
//gradient_x：double
void CSimpleFeatureMatching::gradient(const Mat & img, Mat & gradient_x,unsigned int s)
{
...
void CSimpleFeatureMatching::featureExtract( const Mat & gradient_x,vector<vector<int> > & feaList, double xDiffThresh)
{
...
double CSimpleFeatureMatching::matchCost(const Mat & imgL, const Mat & imgR,int xl,int yl,int xr,int yr,int winSize)
{
...
void CSimpleFeatureMatching ::featureMatching(const Mat & imgL,const Mat & imgR,const vector<vector<int> > & listL,const vector<vector<int> > & listR,vector <vector<int> > & pair)
{
	int H = imgL.rows;
	int W = imgL.cols;
	int halfWinSize = (int)m_winSize/2;
	pair.clear();//清理
	...
	for(int y=0; y<H; y++)
	{
		...
		for(int l=0; l<Nl;l++)
		{
			for(int r=0; r<Nr; r++)
			{
				...
				dist[l][r] = matchCost(imgL, imgR,xl,y,xr,y,m_winSize);
			}
		}
		/*if (y==98)
		{
		...
		for(int l=0; l<Nl; l++)
		{
			...
			if(l == rLover && dist[l][lLover]< m_costThresh) //	&& dist[l][lLover] < 11
			{
				if (lrMinList[l][1] != -1 && dist[l][lrMinList[l][1]]/dist[l][lLover]  < m_peakRatio ) continue;
...
void CSimpleFeatureMatching:: sparseDisparity(const Mat & imgL,const Mat & imgR, Mat & sparseDisp)
{
	Mat imgLGradient(imgL.size(),CV_64FC1,Scalar(0));
	Mat imgRGradient(imgR.size(),CV_64FC1,Scalar(0));

	//计算x方向梯度
	unsigned int s= m_gradientStep;//unsigned int s = 2;
	...
	featureExtract( imgLGradient,listL, m_xDiffThresh);
	featureExtract( imgRGradient,listR, m_xDiffThresh);


	vector <vector<int> > pair;

	featureMatching(imgL,imgR,listL, listR, pair);
